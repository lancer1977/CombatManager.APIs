...
namespace CombatManager
{
    ...
    public sealed class SourceInfo
    {
        ...
        static HashSet<string> _unfoundSources = new HashSet<string>();
        ...
        private static Dictionary<string, NameTypePair> _sourceMap;

        static void AddToMap(SourceType sourceType, List<string> sourceNames)
        {
            AddToMap(sourceType, sourceNames.ToArray<string>());
        }
        ...
        static void AddToMap(string displayName, SourceType sourceType, params string[] sourceNames)
        {
            ...
            _sourceMap.Add(displayName, pair);
        }

        static SourceInfo()
        {
            _sourceMap = new Dictionary<string, NameTypePair>(new InsensitiveEqualityCompararer());
            ...
            AddToMap("Inner Sea Combat", SourceType.ChroniclesAndCompanions);
            AddToMap("Inner Sea Gods", SourceType.ChroniclesAndCompanions);
            AddToMap("Inner Sea Intrigue", SourceType.ChroniclesAndCompanions);
            AddToMap("Inner Sea Monster Codex", SourceType.ChroniclesAndCompanions);
            AddToMap("Inner Sea NPC Codex", SourceType.ChroniclesAndCompanions);
            AddToMap("Inner Sea Races", SourceType.ChroniclesAndCompanions);
            AddToMap("Inner Sea Temples", SourceType.ChroniclesAndCompanions);
            ...
            AddToMap("Pathfinder Bestiary 4", SourceType.Core, "Bestiary 4", "PFRPG Bestiary 4");
            AddToMap("Pathfinder Bestiary 5", SourceType.Core, "Bestiary 5", "PFRPG Bestiary 5");
            AddToMap("Pathfinder Bestiary 6", SourceType.Core, "Bestiary 6", "PFRPG Bestiary 6");
            AddToMap("Monster Codex", SourceType.Core);
            ...
            AddToMap("Ultimate Magic", SourceType.UltimateMagic, "Ultmate Magic", "UM");
            AddToMap("Ultimate Campaign", SourceType.Other);
            AddToMap("Ultimate Intrigue", SourceType.Other);
            ...
            AddToMap("We Be Goblins Too", SourceType.AdventuresAndModules);
            AddToMap("We Be Goblins Free", SourceType.AdventuresAndModules);
            AddToMap("We B4 Goblins", SourceType.AdventuresAndModules);
            AddToMap("We Be 5uper Goblins", SourceType.AdventuresAndModules);
        ...
        private static void AddPfs(int season, int module)

        {
            ...
            if (module < 10)
            {
                names.Add("PFS " + season + "-" + gNum);
            }

            AddToMap(SourceType.AdventuresAndModules, names);
        }

        public static string GetSourceOrDie(string code)
        {
            NameTypePair source;
            if (_sourceMap.TryGetValue(code, out source))
            {
                return source.Name;
            }
        ...
        public static string UnfoundSourcesList
        {
            get
            {
                ...
                foreach (string code in from x in _unfoundSources orderby x select x )
                {
                    list.Append(code);
                    list.Append("\r\n");
                }
