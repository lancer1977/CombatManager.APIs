...
{
    public static Spell.SpellsDbUpdatedDelegate SpellsDbUpdated;
    ...
    static void LoadSpells()
    {
        ...
        if (DbSettings.UseDb)
        {
            _spellsDb = new DatabaseLoader<Spell>("spells.db");

            foreach (Spell s in _spellsDb.GetAll())
            {
                _spells.Add(s);
            }
        }
        ...
        foreach (Spell s in _spells)
        {
            ...
            //add subschool
            if (CmStringUtilities.NotNullString(s.Subschool))
            {
            ...
            if (CmStringUtilities.NotNullString(s.Descriptor))
            {
                //add descriptions
    ...
    private static void RemoveSpellDictionaryItem(Spell s, string oldname)
    {
        ...
        if (_spellDictionary.ContainsKey(checkname))
        {
            ...
            s.PropertyChanged -= Spell_PropertyChanged;
    ...
    private Spell.SpellAdjuster _adjuster;
