...
namespace CombatManager.WPF7.Maps
{
    public class GameMapList : SimpleNotifyClass
    {
        ...
        public delegate void MapChangedDelegate(MapStub map);
        ...
        private void Maps_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (MapStub map in e.NewItems)
                {
            ...
            if (e.OldItems != null)
            {
                foreach (MapStub map in e.OldItems)
                {
                    map.PropertyChanged -= Map_PropertyChanged;
                }
            }
        ...
        public MapFolder GetFolderByPath(IEnumerable<int> folderPath)
        {
            ...
            if (folder == null)
            {
                RootFolder = new MapFolder
                {
                    Maps = new ObservableCollection<MapStub>()
                };
                folder = RootFolder;
            }
        ...
        public class MapStub : SimpleNotifyClass
        {
            ...
            public MapStub(GameMap map)
            {
                ...
                folderPath = new List<int>(map.FolderPath);
                map.PropertyChanged += Map_PropertyChanged;
        ...
        public class MapFolder : SimpleNotifyClass
        {
            ...
            public MapFolder(String name, int id, IEnumerable<int> parentFolderPath)
            {
                this.name = name;
                this.id = id;
                folderPath = new ObservableCollection<int>(parentFolderPath);
                folderPath.Add(id);
