...
namespace CombatManager.WPF7
{
    public class UndoHelper
    {
        ...
        public void AddToUndo(UndoAction act)
        {
            if (!IsOwned)
            {
                if (redoStack.Count > 0)
                {
                    ...
                    redoStack = new LinkedList<UndoAction>();
                }
        ...
        void ClearGroupIfNeccessary()
        {
            ...
            {
                if ((undoStack.Last.Value.UndoType == WPF7.UndoAction.UndoTypes.UndoGroupEnd &&
                    undoStack.Last.Previous.Value.UndoType == WPF7.UndoAction.UndoTypes.UndoGroupStart) ||
                    (undoStack.Last.Value.UndoType == WPF7.UndoAction.UndoTypes.UndoGroupStart &&
                    undoStack.Last.Previous.Value.UndoType == WPF7.UndoAction.UndoTypes.UndoGroupEnd))
                {
        ...
        public List<UndoAction> UndoAction()
        {
            ...
            if (undoStack.Count > 0)
            {
                ...
                if (act.UndoType == WPF7.UndoAction.UndoTypes.UndoGroupEnd)
                {
                    while (true)
                    {
                        if (undoStack.Count == 0)
                        ...
                        if (act.UndoType == WPF7.UndoAction.UndoTypes.UndoGroupStart)
                        {
                            break;
                        }
                    }
